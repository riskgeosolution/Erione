# pages/specific_dash.py (CORRIGIDO: Card "Status Atual" agora usa APENAS o status da Chuva)
...
def get_layout():
    ...
    return dbc.Container([
        ...
        # --- Seção de Relatórios e Eventos (Mantida) ---
        dbc.Row([
            dbc.Col([
...
def update_specific_title(pathname):
    ...
    try:
        ...
        nome_km = config['nome']
        nome_estacao_formatado = f"{nome_km}"
...
def update_specific_dashboard(pathname, dados_json, status_json, selected_hours):
    ...
    df_ponto.loc[:, 'inclinometro_x'] = pd.to_numeric(df_ponto.get('inclinometro_x'), errors='coerce')
    df_ponto.loc[:, 'inclinometro_y'] = pd.to_numeric(df_ponto.get('inclinometro_y'), errors='coerce')

    # --- INÍCIO DA ATUALIZAÇÃO (Usa "chuva" para o Status Atual) ---
    ...
    # O Status Atual (card principal) agora é baseado SOMENTE na chuva
    ...
    try:
        # Pega o último dado de CADA sensor
        ...
        incli_x_str = f"{ultimo_incli_x:.1f}°" if pd.notna(ultimo_incli_x) else "---"
        incli_y_str = f"{ultimo_incli_y:.1f}°" if pd.notna(ultimo_incli_y) else "---"

        layout_cards = [
            # Card 1: Status (2 colunas)
            ...
            # Card 4: Inclinômetro X (2 colunas)
            dbc.Col(dbc.Card(
                dbc.CardBody([html.H5("Inclinômetro X"), html.P(incli_x_str, className="fs-3 fw-bold")]),
                className="shadow h-100 bg-white"), xs=12, md=2, className="mb-4"),

            # Card 5: Inclinômetro Y (2 colunas)
            dbc.Col(dbc.Card(
                dbc.CardBody([html.H5("Inclinômetro Y"), html.P(incli_y_str, className="fs-3 fw-bold")]),
                className="shadow h-100 bg-white"), xs=12, md=2, className="mb-4"),
        ]

    except IndexError as e:
        print(f"Erro de Índice (provavelmente dados vazios): {e}")
        return "Dados insuficientes.", "", id_ponto
    except Exception as e:
        print(f"Erro ao renderizar cards: {e}")
        traceback.print_exc()
        return "Erro ao renderizar cards.", "", id_ponto

    # (Lógica do Gráfico de Chuva)
    ...
    # (Lógica do Gráfico de Inclinômetro)
...
# (O resto dos callbacks - logs, PDF, Excel - permanecem os mesmos da última vez)
# ... (código idêntico) ...
@app.callback(

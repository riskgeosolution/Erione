...
CORES_ALERTAS_CSS = {"verde": "green", "amarelo": "#FFD700", "laranja": "#fd7e14", "vermelho": "#dc3545", "cinza": "grey"}
CORES_UMIDADE = {'1m': CORES_ALERTAS_CSS["verde"], '2m': CORES_ALERTAS_CSS["amarelo"], '3m': CORES_ALERTAS_CSS["vermelho"]}
CORES_INCLINOMETRO = {'Inclinômetro X': '#007BFF', 'Inclinômetro Y': '#E83E8C'}

def get_layout():
    opcoes_tempo_lista = [1, 3, 6, 12, 18, 24, 48, 72, 84, 96]
    opcoes_tempo = [{'label': f'Últimas {h} horas', 'value': h} for h in opcoes_tempo_lista] + [{'label': 'Todo o Histórico (Máx 14 dias)', 'value': 14 * 24}]
        ...
        dbc.Row([
            dbc.Col([
                ...
                dbc.Card(dbc.CardBody([
                    ...
                    dcc.DatePickerRange(id='pdf-date-picker', start_date=(pd.Timestamp.now() - pd.Timedelta(days=7)).date(), end_date=pd.Timestamp.now().date(), display_format='DD/MM/YYYY', className="mb-3 w-100"),
                    html.Br(),
                    html.Div([
...
def update_specific_title(pathname):
    ...
    try:
        ...
        return html.H3(config['nome'], style={'color': '#000000', 'font-weight': 'bold'})
    except Exception:
        return html.H3("Detalhes da Estação", style={'color': '#000000', 'font-weight': 'bold'})

@app.callback([Output('specific-dash-cards', 'children'), Output('specific-dash-graphs', 'children'), Output('store-id-ponto-ativo', 'data')], [Input('url-raiz', 'pathname'), Input('store-dados-sessao', 'data'), Input('store-ultimo-status', 'data'), Input('graph-time-selector', 'value')])
def update_specific_dashboard(pathname, dados_json, status_json, selected_hours):
    ...
    base_1m, base_2m, base_3m = constantes_ponto.get('UMIDADE_BASE_1M', CONSTANTES_PADRAO['UMIDADE_BASE_1M']), constantes_ponto.get('UMIDADE_BASE_2M', CONSTANTES_PADRAO['UMIDADE_BASE_2M']), constantes_ponto.get('UMIDADE_BASE_3M', CONSTANTES_PADRAO['UMIDADE_BASE_3M'])
    try:
    ...
    if df_ponto['timestamp'].dt.tz is None: df_ponto.loc[:, 'timestamp'] = df_ponto['timestamp'].dt.tz_localize('UTC')
    else: df_ponto.loc[:, 'timestamp'] = df_ponto['timestamp'].dt.tz_convert('UTC')
    df_ponto.loc[:, 'timestamp_local'] = df_ponto['timestamp'].dt.tz_convert('America/Sao_Paulo')
    for col in ['chuva_mm', 'precipitacao_acumulada_mm', 'umidade_1m_perc', 'umidade_2m_perc', 'umidade_3m_perc', 'inclinometro_x', 'inclinometro_y']:
        df_ponto.loc[:, col] = pd.to_numeric(df_ponto.get(col), errors='coerce')
    default_status = {"geral": "INDEFINIDO", "chuva": "INDEFINIDO", "umidade": "INDEFINIDO", "inclinometro_x": "INDEFINIDO", "inclinometro_y": "INDEFINIDO"}
    status_ponto_info = status_atual_dict.get(id_ponto, default_status)
    status_card_principal_txt = status_ponto_info.get("chuva", "INDEFINIDO")
    risco_card_principal = RISCO_MAP.get(status_card_principal_txt, -1)
    status_geral_texto, status_geral_cor_bootstrap = STATUS_MAP_HIERARQUICO.get(risco_card_principal, ("INDEFINIDO", "secondary", "bg-secondary"))[:2]
    card_class_color = "bg-" + status_geral_cor_bootstrap if status_geral_cor_bootstrap not in ["warning", "orange", "danger"] else "bg-" + status_geral_cor_bootstrap
    ...
    try:
        ultimo_dado_chuva = df_chuva_72h_completo.iloc[-1]['chuva_mm'] if not df_chuva_72h_completo.empty else 0.0
        ultima_chuva_72h = ultimo_dado_chuva if not pd.isna(ultimo_dado_chuva) else 0.0
        ultimo_dado_umid_valido = df_ponto.dropna(subset=['umidade_1m_perc', 'umidade_2m_perc', 'umidade_3m_perc']).sort_values('timestamp')
        ultimo_umid_1m = ultimo_dado_umid_valido.iloc[-1]['umidade_1m_perc'] if not ultimo_dado_umid_valido.empty else base_1m
        ultimo_umid_2m = ultimo_dado_umid_valido.iloc[-1]['umidade_2m_perc'] if not ultimo_dado_umid_valido.empty else base_2m
        ultimo_umid_3m = ultimo_dado_umid_valido.iloc[-1]['umidade_3m_perc'] if not ultimo_dado_umid_valido.empty else base_3m
        ultimo_dado_incli_valido = df_ponto.dropna(subset=['inclinometro_x', 'inclinometro_y']).sort_values('timestamp')
        ultimo_incli_x = ultimo_dado_incli_valido.iloc[-1]['inclinometro_x'] if not ultimo_dado_incli_valido.empty else pd.NA
        ultimo_incli_y = ultimo_dado_incli_valido.iloc[-1]['inclinometro_y'] if not ultimo_dado_incli_valido.empty else pd.NA
        from config import DELTA_TRIGGER_UMIDADE
        css_color_s1 = CORES_UMIDADE['1m'] if (ultimo_umid_1m - base_1m) >= DELTA_TRIGGER_UMIDADE else 'green'
        css_color_s2 = CORES_UMIDADE['2m'] if (ultimo_umid_2m - base_2m) >= DELTA_TRIGGER_UMIDADE else 'green'
        css_color_s3 = CORES_UMIDADE['3m'] if (ultimo_umid_3m - base_3m) >= DELTA_TRIGGER_UMIDADE else 'green'
        incli_x_str, incli_y_str = f"{ultimo_incli_x:.1f}°" if pd.notna(ultimo_incli_x) else "---", f"{ultimo_incli_y:.1f}°" if pd.notna(ultimo_incli_y) else "---"
        layout_cards = [dbc.Col(dbc.Card(dbc.CardBody([html.H5("Status Atual", style={'color': '#000000', 'font-weight': 'bold'}), html.P(status_geral_texto, className="fs-3", style={'color': '#000000', 'font-weight': 'bold'})]), className=f"shadow h-100 {card_class_color}"), xs=12, md=2, className="mb-4"), dbc.Col(dbc.Card(dbc.CardBody([html.H5("Chuva 72h"), html.P(f"{ultima_chuva_72h:.1f} mm", className="fs-3 fw-bold")]), className="shadow h-100 bg-white"), xs=12, md=2, className="mb-4"), dbc.Col(dbc.Card(dbc.CardBody([html.H5("Umidade do Solo (%)", className="mb-3"), dbc.Row([dbc.Col(html.P([html.Span(f"{ultimo_umid_1m:.1f}", className="fs-3 fw-bold", style={'color': css_color_s1}), html.Span(" (1m)", className="small", style={'color': css_color_s1})], className="mb-0 text-center"), width=4), dbc.Col(html.P([html.Span(f"{ultimo_umid_2m:.1f}", className="fs-3 fw-bold", style={'color': css_color_s2}), html.Span(" (2m)", className="small", style={'color': css_color_s2})], className="mb-0 text-center"), width=4), dbc.Col(html.P([html.Span(f"{ultimo_umid_3m:.1f}", className="fs-3 fw-bold", style={'color': css_color_s3}), html.Span(" (3m)", className="small", style={'color': css_color_s3})], className="mb-0 text-center"), width=4)], justify="around")]), className="shadow h-100 bg-white"), xs=12, md=4, className="mb-4"), dbc.Col(dbc.Card(dbc.CardBody([html.H5("Inclinômetro X"), html.P(incli_x_str, className="fs-3 fw-bold")]), className="shadow h-100 bg-white"), xs=12, md=2, className="mb-4"), dbc.Col(dbc.Card(dbc.CardBody([html.H5("Inclinômetro Y"), html.P(incli_y_str, className="fs-3 fw-bold")]), className="shadow h-100 bg-white"), xs=12, md=2, className="mb-4")]
    except IndexError as e:
        return "Dados insuficientes.", "", id_ponto
    except Exception as e:
        return "Erro ao renderizar cards.", "", id_ponto
    if 'timestamp' in df_chuva_FILTRO_plot.columns:
        if df_chuva_FILTRO_plot['timestamp'].dt.tz is None: df_chuva_FILTRO_plot.loc[:, 'timestamp'] = df_chuva_FILTRO_plot['timestamp'].dt.tz_localize('UTC')
        df_chuva_FILTRO_plot.loc[:, 'timestamp_local'] = df_chuva_FILTRO_plot['timestamp'].dt.tz_convert('America/Sao_Paulo')
    else: df_chuva_FILTRO_plot.loc[:, 'timestamp_local'] = df_chuva_FILTRO_plot['timestamp']
    fig_chuva = make_subplots(specs=[[{"secondary_y": True}]])
    fig_chuva.add_trace(go.Bar(x=df_ponto_plot['timestamp_local'], y=df_ponto_plot['chuva_mm'], name='Pluv. Horária (mm)', marker_color='#2C3E50', opacity=0.8), secondary_y=False)
    fig_chuva.add_trace(go.Scatter(x=df_chuva_FILTRO_plot['timestamp_local'], y=df_chuva_FILTRO_plot['chuva_mm'], name=f'Acumulada ({n_horas_titulo}h)', mode='lines', line=dict(color='#007BFF', width=2.5)), secondary_y=True)
    fig_chuva.update_layout(title_text=f"Pluviometria - {config['nome']} ({n_horas_titulo}h)", template=TEMPLATE_GRAFICO_MODERNO, margin=dict(l=40, r=20, t=50, b=80), legend=dict(orientation="h", yanchor="bottom", y=-0.5, xanchor='center', x=0.5), xaxis_title="Data e Hora", yaxis_title="Pluviometria Horária (mm)", yaxis2_title=f"Acumulada ({n_horas_titulo}h)", hovermode="x unified", bargap=0.1)
    fig_chuva.update_xaxes(dtick=3 * 60 * 60 * 1000, tickformat="%d/%m %Hh", tickangle=-45)
    fig_chuva.update_yaxes(title_text="Pluviometria Horária (mm)", secondary_y=False); fig_chuva.update_yaxes(title_text=f"Acumulada ({n_horas_titulo}h)", secondary_y=True)
    df_umidade = df_ponto_plot.melt(id_vars=['timestamp_local'], value_vars=['umidade_1m_perc', 'umidade_2m_perc', 'umidade_3m_perc'], var_name='Sensor', value_name='Umidade (%)')
    df_umidade['Sensor'] = df_umidade['Sensor'].replace({'umidade_1m_perc': '1m', 'umidade_2m_perc': '2m', 'umidade_3m_perc': '3m'})
    fig_umidade = px.line(df_umidade, x='timestamp_local', y='Umidade (%)', color='Sensor', title=f"Variação da Umidade do Solo - {config['nome']} ({n_horas_titulo}h)", color_discrete_map=CORES_UMIDADE)
    fig_umidade.update_traces(line=dict(width=3)); fig_umidade.update_layout(template=TEMPLATE_GRAFICO_MODERNO, margin=dict(l=40, r=20, t=40, b=80), legend=dict(orientation="h", yanchor="bottom", y=-0.5, xanchor="center", x=0.5), xaxis_title="Data e Hora", yaxis_title="Umidade do Solo (%)")
    fig_umidade.update_xaxes(dtick=3 * 60 * 60 * 1000, tickformat="%d/%m %Hh", tickangle=-45)
    df_inclinometro = df_ponto_plot.melt(id_vars=['timestamp_local'], value_vars=['inclinometro_x', 'inclinometro_y'], var_name='Sensor', value_name='Inclinação (graus)')
    df_inclinometro['Sensor'] = df_inclinometro['Sensor'].replace({'inclinometro_x': 'Inclinômetro X', 'inclinometro_y': 'Inclinômetro Y'})
    fig_inclinometro = px.line(df_inclinometro, x='timestamp_local', y='Inclinação (graus)', color='Sensor', title=f"Inclinômetro - {config['nome']} ({n_horas_titulo}h)", color_discrete_map=CORES_INCLINOMETRO)
    fig_inclinometro.update_traces(line=dict(width=3)); fig_inclinometro.update_layout(template=TEMPLATE_GRAFICO_MODERNO, margin=dict(l=40, r=20, t=40, b=80), legend=dict(orientation="h", yanchor="bottom", y=-0.5, xanchor="center", x=0.5), xaxis_title="Data e Hora")
    fig_inclinometro.update_xaxes(dtick=3 * 60 * 60 * 1000, tickformat="%d/%m %Hh", tickangle=-45)
    layout_graficos = [dbc.Col(dbc.Card(dbc.CardBody(dcc.Graph(figure=fig_chuva)), className="shadow-sm"), width=12, className="mb-4"), dbc.Col(dbc.Card(dbc.CardBody(dcc.Graph(figure=fig_umidade)), className="shadow-sm"), width=12, className="mb-4"), dbc.Col(dbc.Card(dbc.CardBody(dcc.Graph(figure=fig_inclinometro)), className="shadow-sm"), width=12, className="mb-4")]
...
def load_logs_content(is_open, id_ponto, logs_json):
    ...
    try:
        ...
        logs_filtrados_raw = [log for log in logs_ponto_ou_geral if "API: Sucesso" not in log and "Processo Worker (Thread) iniciado" not in log]
        if not logs_filtrados_raw: return f"Nenhum evento crítico ou mudança de status registrada para o ponto {id_ponto}.", []
        ...
        for log_str in reversed(logs_filtrados_raw):
            ...
            cor = "black"
            if "ERRO" in msg_str: cor = "red"
            elif "AVISO" in msg_str: cor = "#E69B00"
            elif "MUDANÇA" in msg_str: cor = "blue"
            logs_formatados_display.append(html.P([html.Strong(f"{timestamp_formatado} [{ponto_str}]: ", style={'color': cor}), html.Span(msg_str, style={'color': cor})], className="mb-1"))
...
def generate_logs_pdf(n_clicks, id_ponto, logs_filtrados):
    if n_clicks is None or not id_ponto or not logs_filtrados: raise PreventUpdate
    try:
        ...
        return dict(content=content_base64, filename=nome_arquivo, base64=True, type="application/pdf")
    except Exception as e:
...
def generate_data_pdf_direct(n_clicks, start_date, end_date, id_ponto, status_json):
...
@app.callback([Output('excel-task-id-store', 'data'), Output('excel-check-interval', 'disabled'), Output('alert-pdf-error', 'is_open', allow_duplicate=True), Output('alert-pdf-generic-error', 'is_open', allow_duplicate=True), Output('report-status-indicator', 'children', allow_duplicate=True), Output('btn-pdf-especifico', 'disabled', allow_duplicate=True), Output('btn-excel-especifico', 'disabled', allow_duplicate=True)], Input('btn-excel-especifico', 'n_clicks'), [State('pdf-date-picker', 'start_date'), State('pdf-date-picker', 'end_date'), State('store-id-ponto-ativo', 'data')], prevent_initial_call=True)
def trigger_excel_generation(n_clicks, start_date, end_date, id_ponto):
    if n_clicks is None or not id_ponto: return dash.no_update, True, False, False, dash.no_update, dash.no_update, dash.no_update
    try:
...
def check_excel_status(n, task_id):
    ...
    with gerador_pdf.EXCEL_CACHE_LOCK: task = gerador_pdf.EXCEL_CACHE.get(task_id)
    if task:
        try:
            if task["status"] == "concluido":
                with gerador_pdf.EXCEL_CACHE_LOCK: del gerador_pdf.EXCEL_CACHE[task_id]
                return dcc.send_bytes(task["data"], task["filename"], type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"), True, False, False, None, False, False
            elif task["status"] == "erro":
                with gerador_pdf.EXCEL_CACHE_LOCK: del gerador_pdf.EXCEL_CACHE[task_id]
                if "Sem dados" in task["message"]: return dash.no_update, True, True, False, None, False, False
                else: return dash.no_update, True, False, True, None, False, False
        except Exception as e:
            with gerador_pdf.EXCEL_CACHE_LOCK:
                if task_id in gerador_pdf.EXCEL_CACHE: del gerador_pdf.EXCEL_CACHE[task_id]
...
def update_dynamic_accumulated_text(selected_hours, dados_json, pathname):
    ...
    try:
        ...
        return html.P(texto_formatado, className="mb-0 ms-3", style={'fontSize': '0.85rem', 'fontWeight': 'bold', 'color': '#555'})
    except Exception as e:
        return None
...
def get_layout():
    ...
    opcoes_tempo = [{'label': f'Últimas {h} horas', 'value': h} for h in opcoes_tempo_lista] + [
        {'label': 'Todo o Histórico (Máx 14 dias)', 'value': 14 * 24}]
...
@app.callback([Output('specific-dash-cards', 'children'), Output('specific-dash-graphs', 'children'),
               Output('store-id-ponto-ativo', 'data')],
              [Input('url-raiz', 'pathname'), Input('store-dados-sessao', 'data'), Input('store-ultimo-status', 'data'),
...
def update_specific_dashboard(pathname, dados_json, status_json, selected_hours):
    ...
    base_1m, base_2m, base_3m = constantes_ponto.get('UMIDADE_BASE_1M',
                                                     CONSTANTES_PADRAO['UMIDADE_BASE_1M']), constantes_ponto.get(
        'UMIDADE_BASE_2M', CONSTANTES_PADRAO['UMIDADE_BASE_2M']), constantes_ponto.get('UMIDADE_BASE_3M',
                                                                                       CONSTANTES_PADRAO[
                                                                                           'UMIDADE_BASE_3M'])
    try:
    ...
    for col in ['chuva_mm', 'precipitacao_acumulada_mm', 'umidade_1m_perc', 'umidade_2m_perc', 'umidade_3m_perc',
    ...
    status_geral_texto, status_geral_cor_bootstrap = STATUS_MAP_HIERARQUICO.get(risco_card_principal,
    ...
    try:
        ...
        ultimo_umid_1m = ultimo_dado_umid_valido.iloc[-1][
            'umidade_1m_perc'] if not ultimo_dado_umid_valido.empty else base_1m
        ultimo_umid_2m = ultimo_dado_umid_valido.iloc[-1][
            'umidade_2m_perc'] if not ultimo_dado_umid_valido.empty else base_2m
        ultimo_umid_3m = ultimo_dado_umid_valido.iloc[-1][
            'umidade_3m_perc'] if not ultimo_dado_umid_valido.empty else base_3m
        ultimo_dado_incli_valido = df_ponto.dropna(subset=['inclinometro_x', 'inclinometro_y']).sort_values('timestamp')
        ultimo_incli_x = ultimo_dado_incli_valido.iloc[-1][
            'inclinometro_x'] if not ultimo_dado_incli_valido.empty else pd.NA
        ultimo_incli_y = ultimo_dado_incli_valido.iloc[-1][
            'inclinometro_y'] if not ultimo_dado_incli_valido.empty else pd.NA
        ...
        layout_cards = [dbc.Col(dbc.Card(dbc.CardBody(
            ...
            className=f"shadow h-100 {card_class_color}"), xs=12, md=2, className="mb-4"),
            dbc.Col(dbc.Card(dbc.CardBody(
                [html.H5("Chuva 72h"), html.P(f"{ultima_chuva_72h:.1f} mm", className="fs-3 fw-bold")]),
                className="shadow h-100 bg-white"), xs=12, md=2, className="mb-4"), dbc.Col(
                dbc.Card(dbc.CardBody([html.H5("Umidade do Solo (%)", className="mb-3"), dbc.Row([dbc.Col(html.P(
                    [html.Span(f"{ultimo_umid_1m:.1f}", className="fs-3 fw-bold", style={'color': css_color_s1}),
                     html.Span(" (1m)", className="small", style={'color': css_color_s1})],
                    className="mb-0 text-center"), width=4), dbc.Col(html.P(
                    [html.Span(f"{ultimo_umid_2m:.1f}", className="fs-3 fw-bold", style={'color': css_color_s2}),
                     html.Span(" (2m)", className="small", style={'color': css_color_s2})],
                    className="mb-0 text-center"), width=4), dbc.Col(html.P(
                    [html.Span(f"{ultimo_umid_3m:.1f}", className="fs-3 fw-bold", style={'color': css_color_s3}),
                     html.Span(" (3m)", className="small", style={'color': css_color_s3})],
                    className="mb-0 text-center"), width=4)], justify="around")]), className="shadow h-100 bg-white"),
                xs=12, md=4, className="mb-4"), dbc.Col(
                dbc.Card(dbc.CardBody([html.H5("Inclinômetro X"), html.P(incli_x_str, className="fs-3 fw-bold")]),
                         className="shadow h-100 bg-white"), xs=12, md=2, className="mb-4"), dbc.Col(
                dbc.Card(dbc.CardBody([html.H5("Inclinômetro Y"), html.P(incli_y_str, className="fs-3 fw-bold")]),
                         className="shadow h-100 bg-white"), xs=12, md=2, className="mb-4")]
    ...
    if 'timestamp' in df_chuva_FILTRO_plot.columns:
        if df_chuva_FILTRO_plot['timestamp'].dt.tz is None: df_chuva_FILTRO_plot.loc[:, 'timestamp'] = \
            df_chuva_FILTRO_plot['timestamp'].dt.tz_localize('UTC')
    ...
    fig_umidade = px.line(df_umidade, x='timestamp_local', y='Umidade (%)', color='Sensor',
                          title=f"Variação da Umidade do Solo - {config['nome']} ({n_horas_titulo}h)",
    ...
    df_inclinometro = df_ponto_plot.melt(id_vars=['timestamp_local'], value_vars=['inclinometro_x', 'inclinometro_y'],
                                         var_name='Sensor', value_name='Inclinação (graus)')
    df_inclinometro['Sensor'] = df_inclinometro['Sensor'].replace(
        {'inclinometro_x': 'Inclinômetro X', 'inclinometro_y': 'Inclinômetro Y'})
    fig_inclinometro = px.line(df_inclinometro, x='timestamp_local', y='Inclinação (graus)', color='Sensor',
                               title=f"Inclinômetro - {config['nome']} ({n_horas_titulo}h)",
                               color_discrete_map=CORES_INCLINOMETRO)
    fig_inclinometro.update_traces(line=dict(width=3));
    fig_inclinometro.update_layout(template=TEMPLATE_GRAFICO_MODERNO, margin=dict(l=40, r=20, t=40, b=80),
                                   legend=dict(orientation="h", yanchor="bottom", y=-0.5, xanchor="center", x=0.5),
    ...
    layout_graficos = [
        ...
        dbc.Col(dbc.Card(dbc.CardBody(dcc.Graph(figure=fig_umidade)), className="shadow-sm"), width=12,
                className="mb-4"),
        dbc.Col(dbc.Card(dbc.CardBody(dcc.Graph(figure=fig_inclinometro)), className="shadow-sm"), width=12,
                className="mb-4")]
...
def load_logs_content(is_open, id_ponto, logs_json):
    ...
    try:
        logs_list = logs_json.split('\n') if isinstance(logs_json, str) else json.loads(logs_json) if isinstance(
            logs_json, str) else logs_list
        ...
        logs_filtrados_raw = [log for log in logs_ponto_ou_geral if
                              "API: Sucesso" not in log and "Processo Worker (Thread) iniciado" not in log]
        ...
        for log_str in reversed(logs_filtrados_raw):
            ...
            logs_formatados_display.append(html.P(
                [html.Strong(f"{timestamp_formatado} [{ponto_str}]: ", style={'color': cor}),
                 html.Span(msg_str, style={'color': cor})], className="mb-1"))
...
def trigger_excel_generation(n_clicks, start_date, end_date, id_ponto):
    if n_clicks is None or not id_ponto: return dash.no_update, True, False, False, dash.no_update, False, False
    try:
        task_id = str(uuid.uuid4())
        ...
        thread.start()
        status_indicator = html.Div(
            [html.Span(dbc.Spinner(size="sm"), className="me-2"), "Gerando Excel, por favor aguarde..."])
        return task_id, False, False, False, status_indicator, True, True
    except Exception as e:
...
def check_excel_status(n, task_id):
    if not task_id: return dash.no_update, True, False, False, dash.no_update, False, False
